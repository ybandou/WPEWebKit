...
const Vector<TVSourceType>&  TVTuner::getSupportedSourceTypes () {
    ...
    if (m_platformTVTuner) {
        ...
        for (auto& tuner : m_platformTVTuner->getSupportedSourceTypes())
            m_sourceTypeList.append(/*ConvertToTVSourceType*/(tuner)); // Add logic to convert this into TVSourceType
    }
...
void  TVTuner::setCurrentSource (TVSourceType sourceType) {
    //m_currentSourceType = sourceType
    if (m_platformTVTuner) {
        // If the voiceList is empty, that's the cue to get the voices from the platform again.
        m_platformTVTuner->setCurrentSource(sourceType);
    }
}
