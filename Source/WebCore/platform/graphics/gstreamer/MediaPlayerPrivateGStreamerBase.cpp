...
void MediaPlayerPrivateGStreamerBase::handleProtectionEvent(GstEvent* event)
{
    ...
    // Currently, the eventKeySystemId in WebM case is null, because the systemID isn't present in the WebM  container.
    // See this commit in GStreamer "event: make system DRM id optional in GST_EVENT_PROTECTION".
    if (m_cdmInstance && eventKeySystemId && strcmp(GStreamerEMEUtilities::keySystemToUuid(m_cdmInstance->keySystem()), eventKeySystemId))
        return;
